...
// Injects precision virtual cursor, smooth zoom engine, and lower-third HUD
async function injectOverlayEngine(page) {
    await page.evaluate(() => {
        if (document.getElementById('scm-virtual-cursor')) return;

        // 1. Precision Virtual Cursor (Tip is anchored precisely at 0, 0)
        const cursor = document.createElement('div');
        cursor.id = 'scm-virtual-cursor';
        cursor.innerHTML = `
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none" style="overflow: visible; filter: drop-shadow(0 3px 6px rgba(0,0,0,0.55));">
                <path d="M0 0 L16 12 L9 13.5 L6 21 L0 0 Z" fill="#0284C7" stroke="#FFFFFF" stroke-width="2.2" stroke-linejoin="round"/>
            </svg>
            <div id="scm-cursor-ripple" style="position: absolute; top: -18px; left: -18px; width: 36px; height: 36px; border-radius: 50%; border: 2.5px solid #38BDF8; opacity: 0; transform: scale(0.3); pointer-events: none; transition: transform 0.5s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.5s ease-out;"></div>
        `;
        ...
        // 2. Lower-Third Glassmorphism Explanatory HUD (Crisp typography & high contrast)
        ...
        `;
        hud.innerHTML = `
            <div style="background: rgba(15, 23, 42, 0.95); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1.5px solid rgba(56, 189, 248, 0.5); border-radius: 20px; padding: 14px 28px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.7); display: flex; align-items: center; gap: 18px; max-width: 880px; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                <div id="scm-hud-badge" style="background: linear-gradient(135deg, #0284C7, #0369A1); color: #FFFFFF; font-size: 11.5px; font-weight: 800; padding: 6px 14px; border-radius: 9999px; text-transform: uppercase; letter-spacing: 0.08em; white-space: nowrap; box-shadow: 0 4px 12px rgba(2,132,199,0.5);">
                    LANGKAH 01 / 12
                </div>
                <div>
                    <div id="scm-hud-title" style="color: #FFFFFF; font-size: 15.5px; font-weight: 700; letter-spacing: -0.01em;">Judul Fitur</div>
                    <div id="scm-hud-desc" style="color: #CBD5E1; font-size: 12.5px; margin-top: 3px; line-height: 1.45;">Deskripsi penjelasan alur operasional sistem.</div>
                </div>
            </div>
        `;
            ...
            try {
                ...
                if (await loc.isVisible({ timeout: 2500 }).catch(() => false)) {
                    ...
                    const rect = await loc.evaluate(el => {
                        ...
                        return {
                            x: isInput ? Math.round(r.left + Math.min(24, r.width / 4)) : Math.round(r.left + r.width / 2),
...
async function recordCinematicDemo() {
    ...
    console.log(`🎬 Launching High-Definition Cinematic Walkthrough (1920x1080 Full HD, Mode: ${isHeaded ? 'HEADED' : 'HEADLESS'})...`);
    ...
    try {
        ...
        await pointTo(page, { x: 500, y: 380 }, { zoom: 1, pause: 1600 });
        // Cursor points precisely to "Katalog Produk"
        await pointTo(page, 'a[href*="/katalog"], button:has-text("Katalog")', { zoom: 1.18, pause: 1800, click: true });
        ...
        await pointTo(page, 'input[value="dp"], label:has-text("DP 50%")', { zoom: 1.22, pause: 1600, click: true });
        ...
        await setCaption(
            page,
            'Langkah 05 / 12',
            'Faktur Tagihan Digital & Perlindungan Pembeli',
            'Dilengkapi QR Code, nomor pesanan unik, serta fitur penyimpanan invoice otomatis di memori browser.'
        );
        // Point to invoice header & status badge
        await pointTo(page, 'span:has-text("Menunggu Pembayaran"), .badge', { zoom: 1.2, pause: 1800 });
        ...
        if (await payBtn.isVisible()) {
            ...
            await setCaption(
                ...
                'Transaksi instan melalui QRIS Nasional (GoPay/ShopeePay/DANA/OVO) atau Virtual Account Bank resmi.'
            );
            await sleep(2800);
        }
        await zoomReset(page, 600);
        ...
        await pointTo(page, '.tracking-milestones, .timeline, h2:has-text("Status")', { zoom: 1.18, pause: 2500 });
        await zoomReset(page, 500);
        ...
        await setCaption(
            ...
            'Visualisasi nilai persediaan stok batu, SPK aktif, rasio kelulusan QC, serta grafik tren produksi bulanan.'
        );
        await pointTo(page, '.grid-cols-4, .kpi-cards, .grid', { zoom: 1.18, pause: 2400 });
        await pointTo(page, '.bg-amber-50, .alert, canvas', { zoom: 1.2, pause: 2200 });
        await zoomReset(page, 500);
        ...
        await pointTo(page, 'table tbody tr:first-child', { zoom: 1.18, pause: 2200 });
        await pointTo(page, 'button:has-text("Verifikasi"), a:has-text("SPK")', { zoom: 1.28, pause: 2000 });
        await zoomReset(page, 500);
        ...
        await pointTo(page, '.kanban-column, .grid-cols-5', { zoom: 1.16, pause: 2500 });
        await zoomReset(page, 500);
        ...
        await pointTo(page, 'canvas, #forecastChart, .chart-container', { zoom: 1.2, pause: 2600 });
        await pointTo(page, 'button:has-text("Hitung"), button:has-text("Forecast")', { zoom: 1.25, pause: 1800 });
        await zoomReset(page, 800);

        console.log('✅ All 12 High-Definition Scenes Recorded Successfully!');

    } catch (err) {
        console.error('❌ Error during Cinematic Demo recording:', err);
    } finally {
        console.log('🎬 Finalizing and saving crystal-clear video file...');
        await page.close();
        ...
        if (videoPath && fs.existsSync(videoPath)) {
            ...
            console.log(`🎉 High-Definition Video saved successfully to: ${targetVideoPath}`);
        }
    }
}
